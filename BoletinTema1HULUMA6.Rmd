...
colnames(datos_2)
#d
commonAmbos<-intersect(rownames(datos),rownames(datos_2))
#e
no<-setdiff(rownames(datos),rownames(datos_2))
no #4 animales
no_2<-setdiff(rownames(datos_2),rownames(datos))
no_2 #7 animales
#Son mamíferos
```
...
#Si que presentan una disposición Gaussiana, ya que son simétricos
```
...
dim(library())
```
4. En las transparencias del tema 1 se citan los primeros pasos a seguir cuando se analiza un nuevov conjunto de datos.
...
#qqplot(data=cabbages, aes(x=VitC, fill=Cult))+geom_histogram(alpha=0.5)
```
